...
public class CommodityServiceImpl extends ServiceImpl<CommodityMapper, Commodity> implements CommodityService {
    ...
    public CommonResult updateCommodity(Long com_id, Integer com_num, Long userId) {
        ...
        //查询是否存在
        Commodity commodity = commodityMapper.selectById(com_id);
        if (commodity == null) return CommonResult.failed("不存在此商品");
        if (commodity.getComStock() < com_num) return CommonResult.failed("商品数量不足");
        //设置
    ...
    public CommonResult updateMainImg(Long com_id, String mainImg, Long userId) {
        ...
        if (!commodity.getMainImage().isEmpty() && !"null".equals(commodity.getMainImage())) {
            //获取原来服务器图片的地址并删除
            //组名
            ...
            String addressName = commodity.getMainImage().substring(
                    commodity.getMainImage().indexOf("/")+1, commodity.getMainImage().length());
            System.out.println("==================");
